...
export const approveRequest = (id) => handler(() => api.patch(`/api/requests/${id}`, { status: 'accepted' }), 'Failed to approve');
export const rejectRequest = (id) => handler(() => api.patch(`/api/requests/${id}`, { status: 'rejected' }), 'Failed to reject');
...
export const getEmployeeData = (email) => handler(() => api.get(`/api/users/${email}`), 'Failed to fetch employee');
export const assignAssetManual = (assetId, employeeEmail) => handler(() => api.post(`/api/assets/${assetId}/assign`, { email: employeeEmail }), 'Failed to assign asset');
